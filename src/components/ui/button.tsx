...
function buttonVariants({ variant, className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "outline" | "ghost" | "destructive" }) {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, ...props }, ref) => {
    ...
    return (
      <button
        className={buttonVariants({ variant, className }) + " " + (sizeClasses[size ?? "default"] ?? "")}
        ref={ref}
        {...props}
      />
    );
