...
**WARNING**:
1. ~~**Home page renders `ƒ Dynamic`, not Static**~~ → **RESOLVED by spec alignment (team decision).** Spec BR "Home page pre-renders with ISR revalidation" updated (commit on `feat/pr3b-public-routes`, propagated to chain) to reflect the Next 14 reality: reading `searchParams` forces per-request Dynamic; the scenario now requires the data layer (posts, categories) to be cached via `unstable_cache` + `revalidate`, which the implementation already does. No code change needed — the design decision was correct, the spec was wrong.
2. ~~**Views increment rewrites `updated_at` via the capture-migration trigger**~~ → **FIXED.** New migration `3_exclude_views_from_updated_at_trigger` (commit `97f2c0d` on `feat/pr3a-foundation`, propagated to chain) gates the trigger with a `WHEN` clause on content columns only (`title`, `slug`, `excerpt`, `content`, `cover_image`, `author_id`, `category_id`, `status`, `reading_time`, `published_at`). Views-only writes no longer touch `updated_at`. Applied to live DB (`migrate deploy`, status up-to-date) and proven at runtime: views-only UPDATE leaves `updatedAt` unchanged; a content UPDATE advances it.
3. **Sitemap omits tag URLs and `/?tag=` filter URLs** — SEO spec "includes … all tag URLs" and design checklist not met (13 URLs only). (unchanged — deferred to the uploads/portadas change per team decision)
...
**PASS WITH WARNINGS — scoped to slice PR3 (post-decision state), merge-ready at the CRITICAL gate.**
...
- WARNING 1 (home Dynamic vs spec): **RESOLVED by spec alignment** — spec now documents Dynamic + cached data layer; no code change.
- WARNING 2 (trigger rewrites `updated_at` on views): **FIXED** — migration 3 gates the trigger on content columns; proven at runtime (views-only update leaves `updatedAt` unchanged).
- WARNING 7 (non-deterministic commits under build-parallel prerender, ~60% aggregate): **ACCEPTED per team decision** — D3 approximate counting; runtime single-page revalidation expected reliable (inferred). No CRITICAL remains open; no global PASS is claimed.
- WARNINGs 3–6 + SUGGESTIONs 1–6 remain open; W3/W4 deferred to the uploads change, W5 paper-trail refresh pending, W6 D6 deviation pending.
